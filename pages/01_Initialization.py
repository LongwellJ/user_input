...
if user and "user_interests" in user:
    # Check specifically for publishers data
    if "publishers" in user["user_interests"]:
        ...
        # User has user_interests but no publishers data
        st.info("We need to update your preferences with publisher information.")
...
# Helper function to parse sources and categorize by publisher
def parse_sources_by_publisher(sources_by_selection):
    publishers_dict = {
        "TC": [],
        "W": [],
        "MG": []
    ...
    # Process each selection and its sources
    for selection_key, sources in sources_by_selection.items():
        for source in sources:
            # Extract publisher info from parentheses
            match = re.search(r'(.*?)\s*\((.*?)\)', source)
            if match:
                source_name = match.group(1).strip()
                publishers = [p.strip() for p in match.group(2).split(',')]
                
                # Map publisher abbreviations to our keys
                for publisher in publishers:
                    if publisher == "TC":
                        if source_name not in publishers_dict["TC"]:
                            publishers_dict["TC"].append(source_name)
                    elif publisher == "Wired":
                        if source_name not in publishers_dict["W"]:
                            publishers_dict["W"].append(source_name)
                    elif publisher == "MG":
                        if source_name not in publishers_dict["MG"]:
                            publishers_dict["MG"].append(source_name)
            else:
                # If no publisher info, just add to all lists (fallback)
                st.warning(f"Source format not recognized: {source}")
...
if st.button("Save My Preferences", type="primary"):
    ...
    if not visible_checked_selections:
        ...
        publishers_dict = parse_sources_by_publisher(user_interests["sources_by_selection"])
        
        # Add the publishers dict to user_interests
        user_interests["publishers"] = publishers_dict
